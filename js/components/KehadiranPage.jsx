...
function KehadiranPage({ murid, kehadiran, setKehadiran, addToast }) {
  ...
  // Auto-sync from school system on first mount
  useEffect(() => {
    if (murid && murid.length > 0) {
      syncKehadiranFromSchool(murid, setKehadiran, null);
    }
  ...
  function getMuridStats(muridId) {
    ...
    let H = 0, T = 0;
    Object.values(recs).forEach(v => { if (v === "H") H++; else if (v === "T") T++; });
    const total = H + T;
    return { H, T, total, pct: total ? getPct(H, total) : 0 };
  }
  ...
  function handleSync() {
    setSyncing(true);
    // Wrap with syncing state reset after toast delay
    const wrappedToast = (msg, type) => {
      addToast(msg, type);
      if (msg.startsWith("✅") || msg.startsWith("❌")) setSyncing(false);
    };
    syncKehadiranFromSchool(murid, setKehadiran, wrappedToast);
  }
    ...
    <div>
      <div className="section-header">
        <div>
          ...
          <div className="section-sub">Data kehadiran diambil terus dari sistem sekolah</div>
        </div>
        <div style={{display:"flex",gap:8}}>
          <button
            ...
            onClick={handleSync}
            disabled={syncing}
            title="Tarik semula data kehadiran dari sistem sekolah"
          >
            🔄 {syncing ? "Menyinkron..." : "Sync Sekolah"}
          </button>
      ...
      <div className="card" style={{marginBottom:16}}>
        <div className="card-body">
          ...
          <div className="legend">
            ...
            <div style={{fontSize:12,color:"var(--text-muted)"}}>* Data diambil automatik dari sistem sekolah</div>
          </div>
        </div>
      </div>
        ...
        <div className="card">
          <div className="card-header">
            ...
            <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
              <button className="btn btn-success btn-sm" style={{pointerEvents:"none",opacity:0.7}}>✅ Hadir (hari)</button>
              <button className="btn btn-danger btn-sm" style={{pointerEvents:"none",opacity:0.7}}>❌ Tidak Hadir</button>
            </div>
          </div>
                  ...
                  return (
                    <tr key={m.id}>
                      ...
                      {days.map(d=>{
                        ...
                        return (
                          <td key={d} className={getCellStyle(val)} title={val||"Tiada rekod"}>
                            {val||"·"}
                          </td>
                        );
                      })}
